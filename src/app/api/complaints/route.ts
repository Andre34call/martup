...
const complaintLimiter = createRateLimiter({ windowMs: 60_000, maxRequests: 5, keyPrefix: 'rl:complaints:' })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const clientIp =
      ...
      'unknown'
    const rateLimit = await complaintLimiter.check(`${authResult.user.id}:${clientIp}`)
